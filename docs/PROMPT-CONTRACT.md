...
- **`POST /v1/pipeline`** runs Stage 1 (resume → aptitude profile), Stage 2 (profile → job types to try), then Stage 3 (plan-driven discovery → aptitude fit ranking → synthesis), with no manual step between stages.
...
| 2 (job types to try) | Yes — `jsonschema` after LLM call |
