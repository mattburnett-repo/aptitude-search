...
Embedding has two sides, same model (config [embedding]):
  ...
  UI / pipeline (runtime, per request):
    Embedding aptitude profile query for aptitude-to-jobtype matching.
  ...
  Occupation corpus (this script, offline):
    Semantic compression layer in corpus for aptitude-to-jobtype matching.
...
Run from repo root (or via data/load-onet-postgres.sh after O*NET load):
  # python data/embed/build_occupation_embeddings.py
...
CREATE_TABLE_SCRIPT = EMBED_DIR / "create-occupation-embeddings-table.sh"
# SQL mashes O*NET rows into one occupation_profile text blob per SOC code.
OCCUPATION_PROFILE_SQL = EMBED_DIR / "occupation_profile_from_onet.sql"
...
def main() -> None:
    ...
    with connect() as conn:
        print("loading occupation profiles from O*NET (occupation_profile_from_onet.sql)...")
