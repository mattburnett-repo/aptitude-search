...
Single-page UI for the three-stage pipeline (resume → aptitude profile → O*NET match + role family plan → verified matches).
...
- **Run pipeline (1 → 2 → 3)** — single request; shows Stage 1, Stage 2 (O*NET occupation matches + role family plan), and Stage 3 verified matches in expandable panels.
- **Save as PDF** — on the aptitude profile, O*NET occupation matches, role family plan, and verified matches panels; opens formatted PDFs in new browser tabs (panel content only; Raw JSON is excluded).
