...
Hugging Face credentials are configured on the API server in `backend/config.toml` (`aptitude_model_key` / `aptitude_model` for Stage 1; `job_discovery_model_key` / `job_discovery_model` for Stage 2). The browser does not send API keys.
...
cp config.example.toml config.toml   # if needed; set [llm].aptitude_model_key and job_discovery_model_key
